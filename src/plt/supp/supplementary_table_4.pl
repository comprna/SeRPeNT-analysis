# #####################################
# supplementary_table_4.pl
#
# Summary of ENCODE cell lines profiles
#
...
# Usage:
#   perl supplementary_table_4.pl [lines|compartments]
# Example
#   perl supplementary_table_4.pl lines > fig/supp/supplementary_table_4.tab
...
# Print values for predicted
foreach my $bio (keys %biotypes) {
  next if ($bio eq "unknown");
  print "Predicted\t$bio";
  foreach my $folder (@folders) {
    my $rounded = sprintf("%.2f", $predicteds{$folder}{$bio} / $profiles{$folder} * 100);
    print "\t$predicteds{$folder}{$bio} ($rounded%)";
  }
  print "\n";
}
